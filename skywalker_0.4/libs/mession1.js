...
import {SpitFire} from "./fighter.js";


export class SceneMission1 extends Scene{
    ...
    constructor(cfgs){
        ...
        this.fighter_bullet_list = [];
        this.enemy_bullet_list = [];
    ...
    init() {
        ...
        let cfgs = ConfigManager.findItemByKey('id','spitfire',ConfigManager.instance.root.fighters);
    ...
    draw(){
        ...
        // print('enemy num:',enemy_list.length);
    ...
    //查询距离玩家最近距离且 y < player.y
    getEnemiesInView(player,params){
        let enemies = [];
        let layer = this.getLayer('enemy');
        for(let n=0;n < layer.sprite_list.length;n++){
            let sprite = layer.sprite_list[n];
            if( sprite.y <=0 || sprite.y >= player.y ){
                //只考虑player上方的敌机
                continue;
            }
            enemies.push(sprite); // just test;
            break ;
        }
        return enemies;
    }
